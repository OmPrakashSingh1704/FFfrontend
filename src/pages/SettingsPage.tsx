...
type RoleAudience = 'founder' | 'investor'
...
const ROLE_FEATURES: Record<RoleAudience, UpcomingFeature[]> = {
  founder: [
    {
      key: 'deal-copilot',
      title: 'Auto Investor Briefs',
      description: 'Generate investor-ready briefs from your pipeline with AI annotations and signal scoring.',
      eta: 'April 2026',
      icon: Sparkles,
      accent: '#fbbf24',
    },
    {
      key: 'warm-intros',
      title: 'Warm Intro Routes',
      description: 'Map mutual connections who can champion your round without leaving FoundersLib.',
      eta: 'May 2026',
      icon: Target,
      accent: '#38bdf8',
    },
    {
      key: 'deck-tracking',
      title: 'Deck Heatmaps',
      description: 'See where investors pause inside your deck to prioritize follow ups.',
      eta: 'June 2026',
      icon: Radar,
      accent: '#a78bfa',
    },
  ...
  investor: [
    {
      key: 'deal-alerts',
      title: 'Realtime Deal Alerts',
      description: 'Pulse updates when founders in your thesis hit new milestones.',
      eta: 'April 2026',
      icon: Radar,
      accent: '#38bdf8',
    },
    {
      key: 'pipeline-sync',
      title: 'CRM Sync',
      description: 'Push accepted intros directly into Affinity, Attio, or HubSpot.',
      eta: 'May 2026',
      icon: Target,
      accent: '#f472b6',
    },
    {
      ...
      description: 'Automatic scoring on every inbound deck using your mandate.',
      eta: 'June 2026',
      icon: Sparkles,
      accent: '#fbbf24',
    },
...
export function SettingsPage() {
  ...
  const roleAudience = useMemo<RoleAudience | null>(() => {
    if (user?.role === 'founder' || user?.role === 'investor') return user.role
    return null
  }, [user?.role])

  const shouldShowUpcoming = roleAudience !== null
  const integrationsReadOnly = shouldShowUpcoming

  const upcomingFeatures = useMemo(() => {
    if (!roleAudience) return []
    return ROLE_FEATURES[roleAudience]
  }, [roleAudience])
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      {shouldShowUpcoming && upcomingFeatures.length > 0 && (
        <div className="card" style={{ marginBottom: '1.5rem' }} data-testid="settings-upcoming-preview">
          <div className="card-header" style={{ alignItems: 'center', gap: '0.75rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <span className="card-title">Upcoming feature preview</span>
              <span style={{ fontSize: '0.8125rem', color: 'hsl(var(--muted-foreground))' }}>
                Because your role is {user?.role ?? 'member'}, here&apos;s what&apos;s launching next. These items are informational only.
              </span>
            </div>
            <span className="badge info" style={{ marginLeft: 'auto' }}>Preview</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {upcomingFeatures.map((feature) => (
              <div
                key={feature.key}
                className="list-item"
                style={{ gap: '0.75rem', cursor: 'default', alignItems: 'flex-start' }}
                data-testid={`upcoming-${feature.key}`}
              >
                <span
                  className="badge"
                  style={{
                ...
                <div style={{ flex: 1 }}>
                  <strong style={{ display: 'block', fontSize: '0.9375rem' }}>{feature.title}</strong>
                  <span style={{ fontSize: '0.8125rem', color: 'hsl(var(--muted-foreground))' }}>{feature.description}</span>
                </div>
                <span className="badge" style={{ backgroundColor: 'transparent', borderColor: 'hsl(var(--border))', color: 'hsl(var(--muted-foreground))' }}>
                  ETA {feature.eta}
                </span>
              </div>
            ))}
        ...
        <>
          ...
          <div className="card" style={{ marginBottom: '1.5rem', position: 'relative', overflow: 'hidden' }}>
            {integrationsReadOnly && (
              <div
                data-testid="connected-blur-overlay"
                style={{
                  position: 'absolute',
                  inset: 0,
              ...
              >
                Integrations are read-only for your role right now.
              </div>
            )}
            <div className="card-header">
            ...
            {loadingAccounts ? (
              ...
              accounts.length === 0 ? (
                <p style={{ fontSize: '0.875rem', color: 'hsl(var(--muted-foreground))', padding: '1rem 0' }}>
                  You haven&apos;t linked external platforms yet. Connection controls will unlock once this preview graduates.
                </p>
              ) : (
                <ul
                  data-testid="connected-readonly-list"
                  style={{
                    listStyle: 'disc',
                    paddingLeft: '1.25rem',
                    display: 'flex',
                ...
                >
                  {accounts.map((account) => (
                    <li key={account.id} style={{ fontSize: '0.875rem', lineHeight: 1.4 }}>
                      <strong>{account.display_name ?? PLATFORM_LABELS[account.platform] ?? account.platform}</strong>
                      {account.external_email && (
                        <span style={{ marginLeft: 8, color: 'hsl(var(--muted-foreground))' }}>{account.external_email}</span>
                      )}
                      {account.is_active === false && (
                        <span style={{ marginLeft: 8, color: '#f97316', fontSize: '0.8125rem' }}>Inactive</span>
                      )}
                      {account.error_message && (
                        <span style={{ display: 'block', color: '#ef4444', fontSize: '0.8125rem' }}>{account.error_message}</span>
                      )}
                    </li>
                  ))}
                </ul>
          ...
          <div className="card" style={{ position: 'relative', overflow: 'hidden' }}>
            {integrationsReadOnly && (
              <div
                data-testid="integrations-blur-overlay"
                style={{
                  position: 'absolute',
                  inset: 0,
                  background: 'rgba(2, 4, 8, 0.65)',
              ...
              >
                Integrations are in preview for your role. Connection controls will unlock soon.
              </div>
            )}
            <div className="card-header">
